...
parser.add_argument("--mod_dir", type=str, default='WGAN_GP_2', help="directory of model")
parser.add_argument("--image_size", type=int, default=32, help="directory of model")
parser.add_argument("--train_epoch", type=int, default=128, help="number of epochs of training")
...
def train_it(
    ...
    for real_images in train_loader:
        ...
        loss_disc = fake_disc.mean() - real_disc.mean() + lambda_gp * gradient_penalty
        loss_disc.backward()
        optim_disc.step()
...
def run_train(mod_dir, gpu_id,
    ...
    '''train model'''
    generator = Generator(opt.latent_dim, 48, 3).to(device)
    discriminator = Discriminator(3, 48, 1).to(device)
    optim_gene = torch.optim.Adam(generator.parameters(), lr=4e-4, betas=(0.75, 0.99))
    optim_disc = torch.optim.Adam(discriminator.parameters(), lr=2e-3, betas=(0.75, 0.99))
    # criterion = None

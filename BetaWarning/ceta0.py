...
"""
ISAC1101
ceta1 Minitaur  # fix, reward_scale = 2 ** 6
ceta4 Minitaur  # fix, reward_scale = 2 ** 5 ---R10

beta1 # fix, LLBW

beta0 - a_std_log.abs() + fix_term / 8, reward_scale = 2 ** 5
beta2 - a_std_log.abs() + fix_term / 8, reward_scale = 2 ** 7
beta3 - a_std_log.abs() + fix_term / 8, reward_scale = 2 ** 5, net, batch size

alpha_lr
ceta2 - a_std_log.abs(), reward_scale = 2 ** 5
ceta3 - a_std_log.abs(), reward_scale = 2 ** 5, net, batch size
ceta4, cancel fix_term
"""
...
class InterSPG1101(nn.Module):  # class AgentIntelAC for SAC (SPG means stochastic policy gradient)
    ...
    def get__a__log_prob_1101(self, state):  # actor
        ...
        # log_prob = noise.pow(2) * 0.5 - a_std_log.abs() + fix_term / 8  # todo Minitaur 18 # / 8,  Minitaur 24
        log_prob = noise.pow(2) * 0.5 + a_std_log

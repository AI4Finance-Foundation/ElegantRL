...
Auto TTUR
Adaptive lambda
Adam regular term actor_term beta13
Adam regular term actor_term  cancel beta0
"""
...
class InterSPG(nn.Module):  # class AgentIntelAC for SAC (SPG means stochastic policy gradient)
    ...
    def get__a__std(self, state):
        ...
        a_mean = self.dec_a(a_)  # NOTICE! it is a_mean without .tanh()
        a_std_log = self.dec_d(a_).clamp(self.log_std_min, self.log_std_max)
        return a_mean.tanh()  # , a_std_log todo
...
class AgentInterSAC1(AgentBasicAC):  # Integrated Soft Actor-Critic Methods
    ...
    def update_parameters(self, buffer, max_step, batch_size, repeat_times):
        ...
        for n_c in range(1, update_times):
            ...
            a1_mean, a_noise, log_prob = self.act.get__a__avg_std_noise_prob(state)  # policy gradient
            # todo

            '''action correction term'''
            # a2_mean, a2_log_std = self.act_target.get__a__std(state)
            a2_mean = self.act_target.get__a__std(state)  # todo
            actor_term = self.criterion(a1_mean, a2_mean)  # + self.criterion(a1_log_std, a2_log_std)  # todo
            # actor_term = self.criterion(a1_mean, a2_mean)
        ...
        # print(f'{self.avg_loss_c:8.3e}   {alpha.item():8.3e}    '
        #       f'{log_prob.mean().item():8.3e}   {self.target_entropy:8.3e}')
...
def run_continuous_action(gpu_id=None):
    ...
    args = Arguments(rl_agent=AgentInterSAC1, gpu_id=gpu_id)

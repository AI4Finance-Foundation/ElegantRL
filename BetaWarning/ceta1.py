...
def modify_log_prob(a_mean, a_std, a_std_log, self):
    noise = torch.randn_like(a_mean, requires_grad=True, device=self.device).clamp(-2, 2)* a_std
    a_noise = a_mean + noise
    ...
    # # fix_term = ((a_noise_tanh - a_mean.tanh())/noise + 0.00001).log()
    # log_prob = log_prob_noise + fix_term

    log_prob_noise = (noise / a_std).pow(2) + a_std_log.clamp_max(0.9)
...
class InterSPG(nn.Module):  # class AgentIntelAC for SAC (SPG means stochastic policy gradient)
    ...
    def get__a__log_prob(self, state):  # actor
        ...
        # return a_noise_tanh, log_prob.sum(1, keepdim=True)
        return modify_log_prob(a_mean, a_std, a_std_log, self)
    ...
    def get__a__avg_std_noise_prob(self, state):  # actor
        ...
        res1, res2 = modify_log_prob(a_mean, a_std, a_std_log, self)

...
def train__car_racing(gpu_id=None, random_seed=0):
    ...
    env = gym.make('CarRacing-v0')  # todo don't penalize "die state" too much in AgentRun.py
    ...
    args.break_step = int(5e5 * 4)  # (2e5) 5e5, UsedTime (15000s) 25000s
    args.reward_scale = 2 ** -2  # (-1) 50 ~ 800 ~ 900 (1001)
    args.gamma = 0.95  # todo
    args.max_memo = 2 ** 11
    args.batch_size = 2 ** 7
...
train__car_racing(random_seed=1234)

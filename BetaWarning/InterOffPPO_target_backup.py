...
class InterPPO(nn.Module):
    ...
    def get__q__log_prob(self, state, noise):
        x = self.net(state)
        new_v = self.dec_q(x)
        log_prob = -(noise.pow(2) / 2 + self.net__std_log).sum(1)
        return new_v, log_prob
    ...
    def __init__(self, state_dim, action_dim, net_dim):
        ...
        self.act_optimizer = torch.optim.Adam(
            [{'params': self.act.net.parameters(), 'lr': self.learning_rate * 0.5},
             ...
             {'params': self.act.net__std_log, }, ],  # todo fix bug
            lr=self.learning_rate, )

        self.act_target = InterPPO(state_dim, action_dim, net_dim).to(self.device)
    ...
    def update_buffer(self, env, buffer, max_step, reward_scale, gamma):
        ...
        while step_counter < max_memo:
            ...
            for step_sum in range(max_step):
                ...
                action = a_avg + noise * noise_std  # todo pure_noise
    ...
    def update_policy(self, buffer, _max_step, batch_size, repeat_times):
        ...
        with torch.no_grad():
            ...
            for i in range(0, all_state.size()[0], b_size):
                new_v, log_prob = self.act_target.get__q__log_prob(
                    all_state[i:i + b_size],
                    all_noise[i:i + b_size],
                )
        ...
        for _ in range(sample_times):
            ...
            self.avg_loss_c = 0.995 * self.avg_loss_c + 0.005 * critic_loss.item() / 4  # soft update, twin critics
            lamb = np.exp(-self.avg_loss_c ** 2)

            '''actor_loss'''
            a_avg = self.act.dec_a(x)  # todo fix bug
        ...
        return a_log_std.mean().item(), critic_loss.item()  # todo

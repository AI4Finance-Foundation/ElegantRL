...
"""
New version InterSAC 2020-09-09 (solve log_alpha trouble)
"""


class AgentInterSAC2(AgentBasicAC):  # Integrated Soft Actor-Critic Methods
    def __init__(self, state_dim, action_dim, net_dim):
        ...
        self.neg_log_alpha = torch.zeros(1, requires_grad=True, device=self.device)
        self.alpha = -self.neg_log_alpha.exp()
        self.alpha_optimizer = torch.optim.Adam((self.neg_log_alpha,), lr=self.learning_rate)
    ...
    def update_parameters(self, buffer, max_step, batch_size, repeat_times):
        ...
        for i in range(update_times):
            ...
            alpha_loss = (self.neg_log_alpha * (self.target_entropy - log_prob).detach()).mean()
            self.alpha_optimizer.zero_grad()
            alpha_loss.backward()
            self.alpha_optimizer.step()
            self.alpha = -self.neg_log_alpha.exp()  # todo
...
class AgentInterSAC3(AgentBasicAC):  # Integrated Soft Actor-Critic Methods
    def __init__(self, state_dim, action_dim, net_dim):
        ...
        '''extension: auto-alpha for maximum entropy'''
        # self.log_neg_alpha = torch.zeros(1, requires_grad=True, device=self.device)
        # self.alpha = -self.log_neg_alpha.exp()
        # self.alpha_optimizer = torch.optim.Adam((self.log_neg_alpha,), lr=self.learning_rate)
    ...
    def update_parameters(self, buffer, max_step, batch_size, repeat_times):
        ...
        for i in range(update_times):
            ...
            '''auto alpha'''
            # alpha_loss = -(self.log_alpha * (log_prob - self.target_entropy).detach()).mean()
            alpha_loss = ((-self.alpha).log() * (self.target_entropy - log_prob).detach()).mean()
            self.alpha_optimizer.zero_grad()
            alpha_loss.backward()
            self.alpha_optimizer.step()
            # self.alpha = -self.log_neg_alpha.exp()  # todo

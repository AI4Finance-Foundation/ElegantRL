...
def mp__update_params(args, q__net, q__buf, q__eva, ):  # update network parameters using replay buffer
    ...
    q__net.put(act_cpu)  # q__net 1.
    q__eva.put(act_cpu)  # q__eva 1.
    ...
    start_time = timer()
    for epoch in range(int(2 ** 12)):  # epoch is episode # todo
        ...
        q__net.put(act_cpu)  # q__net n.
        q__eva.put((act_cpu, reward_avg, step_sum, loss_a_avg, loss_c_avg))  # q__eva n.
...
def mp__update_buffer(args, q__net, q__buf):  # update replay buffer by interacting with env
    ...
    act = q__net_get  # q__net_get == act.to(device_cpu), requires_grad=False
...
def mp__evaluate_agent(args, q__eva, ):  # evaluate agent and get its total reward of an episode
    ...
    act = q__eva_get  # q__eva_get == act.to(device_cpu), requires_grad=False

    print(f"{'GPU':3}|"
          f"{'ExpR':>8}  {'ExpS':>8}  {'LossA':>9}  {'LossC':>9}|"
          f"{'R avg':>8}  {'R std':>8}")
    while q__eva_get is not None:
        '''update actor'''
        q__eva_get = q__eva.get()  # q__eva n.
        act, exp_r_avg, exp_s_sum, loss_a_avg, loss_c_avg = q__eva_get
        ...
        print(f'{gpu_id:<3}|'
              f'{exp_r_avg:8.2f}  {exp_s_sum:8.2e}  {loss_a_avg:9.2e}  {loss_c_avg:9.2e}|'

...
class AgentSAC0:
    ...
    def inactive_in_env_sac(self, env, memo, max_step, max_action, reward_scale, gamma):
        ...
        for t in range(max_step):
            ...
            memo.add_memo(self.state, action, res_reward, next_state, mask)
        ...
        for _ in range(iter_num):
            with torch.no_grad():
                ...
                states, actions, rewards, next_states, marks = [
                    ...
                    for ary in memo.random_sample(batch_size)
                ]
...
class AgentSAC1:
    def __init__(self, env, state_dim, action_dim, net_dim):
        ...
        cri_dim = int(net_dim * 1.25)
        self.cri = CriticTwin(state_dim, action_dim, cri_dim, use_densenet, use_spectral_norm).to(self.device)
        self.cri_optimizer = torch.optim.Adam(self.cri.parameters(), lr=self.learning_rate)
        self.cri_target = copy.deepcopy(self.cri).to(self.device)
    ...
    def update_memory(self, env, memo, max_step, max_action, gamma, reward_scale):
        rewards = list()
        steps = list()
        for t in range(max_step):
            action = self.select_actions((self.state,), explore_noise=True)[0]
            ...
            mask = 0.0 if done else gamma
            memo.add_memo((adjust_reward, mask, self.state, action, next_state))
    ...
    def update_parameter(self, memo, max_step, batch_size):
        loss_a_sum = 0.0
        loss_c_sum = 0.0
        iter_num = max_step
        for _ in range(iter_num):
            with torch.no_grad():
                tensors = memo.random_sample(batch_size, self.device)
                rewards, masks, states, actions, next_states = tensors
            ...
            '''actor loss'''
            alpha = self.log_alpha.exp()
            q0_min = torch.min(*self.cri.get_q1_q2(states, actions_noise))
            actor_loss = (alpha * log_prob - q0_min).mean()
            loss_a_sum += actor_loss.item()
            ...
            '''critic loss'''
            q1_eval, q2_eval = self.cri.get_q1_q2(states, actions)
            with torch.no_grad():
                next_actions_noise, next_log_prob_noise = self.act.get__a__log_prob(next_states)

                next_q0_min = torch.min(*self.cri_target.get_q1_q2(next_states, next_actions_noise))
                next_target_q_value = next_q0_min - next_log_prob_noise * alpha
                q0_target = rewards + masks * next_target_q_value

            qf1_loss = self.criterion(q1_eval.expand_as(q0_target), q0_target)
            qf2_loss = self.criterion(q2_eval.expand_as(q0_target), q0_target)
            critic_loss = qf1_loss + qf2_loss
            # q0_target_concat = torch.cat((q0_target, q0_target), dim=0)
            # q0_eval_concat = torch.cat((q1_eval, q2_eval), dim=0).expand_as(q0_target_concat)
            # critic_loss = self.criterion(q0_eval_concat, q0_target_concat)
            loss_c_sum += critic_loss.item()
...
class AgentSAC:
    def __init__(self, env, state_dim, action_dim, net_dim):
        ...
        self.cri2 = CriticSAC(state_dim, action_dim, cri_dim, use_densenet, use_spectral_norm).to(self.device)
        self.cri_optimizer2 = torch.optim.Adam(self.cri2.parameters(), lr=self.learning_rate)

        self.cri_target = copy.deepcopy(self.cri).to(self.device)
        self.cri_target2 = copy.deepcopy(self.cri2).to(self.device)
    ...
    def update_parameter(self, memo, max_step, batch_size):
        ...
        for _ in range(iter_num):
            ...
            '''critic loss'''
            q1_eval = self.cri(states, actions)
            q2_eval = self.cri2(states, actions)

            with torch.no_grad():
                next_actions_noise, next_log_prob_noise = self.act.get__a__log_prob(next_states)

                next_q0_min = torch.min(
                    self.cri_target(next_states, next_actions_noise),
                    self.cri_target2(next_states, next_actions_noise)
                )
                next_target_q_value = next_q0_min - next_log_prob_noise * alpha
                q0_target = rewards + next_target_q_value * masks  # mask == gamma * (1-done)

            critic_loss = (q1_eval - q0_target).pow(2).mean()
            # critic_loss = self.criterion(q1_eval, q0_target)
            self.cri_optimizer.zero_grad()
            critic_loss.backward()
            self.cri_optimizer.step()

            critic_loss2 = (q2_eval - q0_target).pow(2).mean()
            self.cri_optimizer2.zero_grad()
            critic_loss2.backward()
            self.cri_optimizer2.step()

            loss_c_sum += (critic_loss.item() + critic_loss2.item()) * 0.5
            with torch.no_grad():
                self.soft_target_update(self.cri_target, self.cri)
...
class Memories:  # todo del
    def __init__(self, max_len):
        self.memories = list()
        self.max_len = max_len
        self.now_len = len(self.memories)
        self.now_ind = self.now_len
    ...
    def del_memo(self):
        ...
        if del_len > 0:
            del self.memories[:self.memories]
    ...
    def random_sample(self, batch_size):
        indices = rd.choice(len(self.memories), batch_size, replace=False)

        if self.now_ind > 0:
            self.now_ind -= 1
            indices[0] = self.now_ind

        arrays = [list() for _ in self.memories[0]]
        # for indice in indices:
        #     for item


class MemoryList:  # todo del
    def __init__(self, ):
        self.memory = []
...
class MemoryArray:  # Experiment Replay Buffer 2020-04-04
    ...
    def random_sample(self, batch_size, device):
        ...
        # '''add recent memory into training batch'''
        # self.now_idx = (self.now_idx - 1) % self.max_len
        # indices[0] = self.now_idx
...
class Recorder:
    ...
    def show_reward(self, epoch_rewards, iter_numbers, actor_loss, critic_loss):
    ...
    def check_reward(self, cwd,):  # 2020-05-05
        is_solved = False
        if self.reward_avg >= self.reward_max:  # and len(self.rewards) > 1:  # 2020-04-30
            ...
            print("{:4} |{:8} {:8.2f} {:8.2f} {:8.2f} |{:8} {:6} |{:.2e}".format(
                len(self.record_epoch),

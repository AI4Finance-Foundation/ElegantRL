...
class Arguments:
    ...
    random_seed = 19430535  # int(gpu_id)
    ...
    soft_update_tau = 0.5  # could be 0.005
    iter_num_k = 2
    ...
    def __init__(self):
        # self.env_name = "LunarLanderContinuous-v2"  # 1397s 77e, 2423s 69e
        # # 1191s 45e, 1373s 54e, 1622s 55e

        # self.env_name = "BipedalWalker-v2"  # 1887s 94e, 2245s 123e, 1686s 93e
        ...
        # self.env_name = "BipedalWalkerHardcore-v2"
        # # self.explore_noise = 0.1
        # # self.policy_noise = 0.4
        # # self.memories_size = int(2 ** 16)  # memories capacity (memories: replay buffer)
        # self.update_gap = int(2 ** 8)  # update the target_net, delay update
        # self.is_remove = None
        # self.max_epoch = 4000  # max num of train_epoch


'''train'''


def run_train():  # 2019-11-19
    args = Arguments()
    ...
    '''train loop'''
    recorders = list()  # [(eva_reward, epoch_reward, actor_loss, critic_loss), ]
    ...
    try:
        for epoch in range(max_epoch):
            ...
            actor_loss, critic_loss = agent.update_parameter(
                memories, iter_num, batch_size, policy_noise, iter_num_k, gamma, update_gap,
                soft_update_tau, epoch)
            eva_reward = get_eva_reward(agent, env, max_step, action_max, action_dim, target_reward)
...
def run_test():  # 2019-11-23
    ...
    state = env.reset()
    print(state.shape, np.max(state))

    for i in range(128):
        action = np.array([0.01, 1, 0])
        state, reward, done, _ = env.step(action)
        env.render(mode='human')  # assert mode in ['human', 'state_pixels', 'rgb_array']
        # state = state2d_1d(state)
        img = state[:, :, 1]
        # img = cv2.blur(img, (2, 2))
        # img = cv2.resize(img, (40, 40))  # 1600
...
class AgentDelayDDPG:
    ...
    def inactive_in_env(self, env, memories, max_step, explore_noise, action_max):
        ...
        for iter_num in range(max_step):
            ...
            next_state = state2d_1d(next_state)  # For CarRacing-v0
            if np.sum(np.abs(state - next_state)) == 0 or np.sum(next_state > 1.1) > 185:  # For CarRacing-v0
                # stop, outside  # For CarRacing-v0
                reward = -100  # For CarRacing-v0
                done = True  # For CarRacing-v0
    ...
    def update_parameter(self, memories, iter_num, batch_size, policy_noise,
                         iter_num_k, gamma, update_gap, soft_update_tau, epoch):  # 2019-11-21
        self.act_optimizer.param_groups[0]['lr'] = \
            np.exp(-(self.critic_loss_avg / 4) ** 2) * 4e-4
        ...
        self.critic_loss_avg /= iter_num * iter_num_k
...
class DenseNet(nn.Module):
    def __init__(self, inp_dim, mid_dim, out_dim, sn=False):
        ...
        # self.dense3 = (nn.Linear(mid_dim * 4, mid_dim * 4))
        ...
        #         nn.BatchNorm1d(inp_dim * 1, eps=1e-5, momentum=1 - 1e-5),
        #         self.dense0)
    ...
    def forward(self, x0):
        ...
        # x4 = torch.cat((x3, f_hard_swish(self.dense3(x3))), dim=1)
...
class Actor(nn.Module):
    ...
    def forward(self, x, noise=0):
        ...
        if noise:
            with torch.no_grad():
                n = torch.randn_like(a, dtype=torch.float32).cuda() * noise
                n = torch.tanh(n)
                a1 = a + n
                m = ((-1.0 < a1) + (a1 < 1.0)).type(torch.float32)
                n *= m
            a += n
        return a
...
class Critic(nn.Module):
    def __init__(self, state_dim, action_dim, mid_dim):
        ...
        self.net = DenseNet(inp_dim=state_dim + action_dim, mid_dim=mid_dim, out_dim=1,
                            sn=True)
...
def draw_plot(recorders, smooth_kernel, mod_dir, save_name=None):  # 2019-11-26

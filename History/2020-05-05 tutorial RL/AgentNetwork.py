...
class ActorDN(nn.Module):  # dn: DenseNet
    def __init__(self, state_dim, action_dim, mid_dim, use_dn):
        ...
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.net = build_actor_network(state_dim, action_dim, mid_dim, use_dn)
...
class ActorPPO(nn.Module):
    def __init__(self, state_dim, action_dim, mid_dim):
        ...
        self.net__mean = nn.Sequential(nn.BatchNorm1d(state_dim),
                                       nn.Linear(state_dim, mid_dim), nn.ReLU(),
                                       nn.Linear(mid_dim, mid_dim), nn.ReLU(),
        ...
        layer_norm(self.net__mean[1], std=1.0)
        layer_norm(self.net__mean[3], std=1.0)
        layer_norm(self.net__mean[5], std=0.01)  # output layer for action
    ...
    def get__a__log_prob(self, a_mean):
        ...
        log_prob = -(a_log_std + (a_noise - a_mean).pow(2) / (2 * a_std.pow(2)) + self.constant_log_sqrt_2pi)
        log_prob = log_prob.sum(1)
        return a_noise, log_prob

    def compute__log_prob(self, a_mean, a_noise):
        a_log_std = self.net__std_log.expand_as(a_mean)
        a_std = torch.exp(a_log_std)
        log_prob = -(a_log_std + (a_noise - a_mean).pow(2) / (2 * a_std.pow(2)) + self.constant_log_sqrt_2pi)
    ...
    #     a_log_std = self.net__std_log.expand_as(a_mean)
    #     a_std = torch.exp(a_log_std)
    #     log_prob = -(a_log_std + (a_inp - a_mean).pow(2) / (2 * a_std.pow(2)) + self.constant_log_sqrt_2pi)
    #     log_prob = log_prob.sum(1)
    #     return log_prob
...
class CriticTwin(nn.Module):  # TwinSAC <- TD3(TwinDDD) <- DoubleDQN <- Double Q-learning
    def __init__(self, state_dim, action_dim, mid_dim):
        ...
        self.net1 = build_critic_network(state_dim, action_dim, mid_dim, use_dn=False, use_sn=False)
        self.net2 = build_critic_network(state_dim, action_dim, mid_dim, use_dn=False, use_sn=False)
...
class CriticAdvantage(nn.Module):  # 2020-05-05 fix bug
    def __init__(self, state_dim, mid_dim):
        ...
        self.net = nn.Sequential(nn.BatchNorm1d(state_dim),
                                 nn.Linear(state_dim, mid_dim), nn.ReLU(),
                                 nn.Linear(mid_dim, mid_dim), nn.ReLU(),
                                 nn.Linear(mid_dim, 1), )

        layer_norm(self.net[1], std=1.0)
        layer_norm(self.net[3], std=1.0)
        layer_norm(self.net[5], std=1.0)  # output layer for action
...
def build_actor_network(state_dim, action_dim, mid_dim, use_dn):
    nn_list = list()

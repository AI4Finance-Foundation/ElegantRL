...
class ActorSAC(nn.Module):
    ...
    def get__a__log_prob(self, states):
        ...
        # log_prob_noise = log_prob_mean - (1 - a_noise_tanh.pow(2) + 1e-6)).log()
        # log_prob_noise = log_prob_noise.sum(dim=1, keepdim=True)
        # return a_noise_tanh, log_prob_noise
        log_prob_mean = -(a_std.log() + self.constant_pi + ((a_noise - a_mean) / a_std).pow(2) * 0.5)
        log_prob_noise = log_prob_mean - (-a_noise_tanh.pow(2) + 1.000001).log()
        log_prob_noise = log_prob_noise.sum(dim=1, keepdim=True)
        return a_noise_tanh, log_prob_noise

...
LunarLanderContinuous-v2: 1231s E114
...
class Arguments:
    ...
    '''algorithm'''
    action_num = 2  # Actor Network outputs multi-action.
...
def f_hard_swish(x):
    return ff.relu6(x + 3) / 6 * x


class Actor(nn.Module):
    def __init__(self, mod_dim, state_dim, action_dim, action_num):
        super(Actor, self).__init__()
        inp_dim = state_dim
        out_dim = action_dim * action_num
    ...
    def forward(self, x0):
        ...
        x3 = ff.dropout(x3, p=rd.uniform(0.0, 0.5), training=self.training)
        x4 = torch.tanh(self.dense3(x3))
        # return x4.view(-1, self.action_num, self.action_dim)
        return x4


class Critic(nn.Module):
    def __init__(self, mod_dim, state_dim, action_dim):
...
class AgentDelayDDPG:
    def __init__(self, args, state_dim, action_dim, action_num):
        ...
        self.act = Actor(mod_dim, state_dim, action_dim, action_num).to(self.device)
        # self.act_optimizer = optim.Adam(self.act.parameters(), lr=4e-4)
        self.act_optimizer = Lookahead(optim.Adam(self.act.parameters(), lr=4e-4),
                                       k=5, alpha=0.5)
        self.act.train()

        self.act_target = Actor(mod_dim, state_dim, action_dim, action_num).to(self.device)
        ...
        self.cri_optimizer = Lookahead(optim.Adam(self.cri.parameters(), lr=1e-3),
                                       k=5, alpha=0.5)
        self.cri.train()

        self.cri_target = Critic(mod_dim, state_dim, action_dim).to(self.device)
...
class Memories:
    ...
    def __init__(self, memories_num, state_dim, action_dim, action_num):
        self.size = 0
        ...
        reward_dim = 1
        done_dim = 1
        memories_dim = reward_dim + done_dim + state_dim + action_dim * action_num + state_dim
...
def train():
    ...
    agent = AgentDelayDDPG(args, state_dim, action_dim, action_num)

    memories = Memories(memories_size, state_dim, action_dim, action_num)
    ...
    try:
        for epoch in range(max_epoch):
            ...
            for iter_num in range(max_step):
                ...
                actions += rd_normal(0, explore_noise, size=actions.shape)  # add explore noise
                actions = actions.clip(-1.0, 1.0)

                for action in actions.reshape((action_num, action_dim)):
                    next_state, reward, done, _ = env.step(adapt_action(action, action_max, action_dim, is_train=True))
                    memories.add(np.hstack(((reward, 1 - float(done)), state, actions, next_state)))
                    state = next_state
            ...
            al, cl = agent.update(memories, iter_num, batch_size)
                    ...
                    for _ in range(max_step):
                        ...
                        for action in actions.reshape((action_num, action_dim)):
                            state, reward, done, _ = env.step(
                                adapt_action(action, action_max, action_dim, is_train=False))
                            eva_reward += reward
                            # env.render()
                            if done:
                                break
...
def evals():
    ...
    '''mod init'''
    os.environ['CUDA_VISIBLE_DEVICES'] = str(gpu_id)
    policy = AgentDelayDDPG(args, state_dim, action_dim, action_num)

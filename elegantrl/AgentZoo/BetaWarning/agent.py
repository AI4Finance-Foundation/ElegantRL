...
class AgentBase:
    ...
    def select_action(self, state):
        ...
        return action.detach().cpu().numpy()  # -1 < action < +1
    ...
    def select_actions(self, states):  # states = (state, ...)
        ...
        return actions.detach().cpu().numpy()  # -1 < action < +1
...
class AgentDQN(AgentBase):
    ...
    def select_action(self, state):  # for discrete action space
        ...
        :param state: (state, ) or (state, state, ...) or state.shape==(n, *state_dim)
        """
        if rd.rand() < self.explore_rate:  # epsilon-greedy
            ...
            states = torch.as_tensor((state,), dtype=torch.float32, device=self.device)
            action = self.act(states)[0]
            a_int = action.argmax(dim=1).detach().cpu().numpy()
        return a_int
...
class AgentDoubleDQN(AgentDQN):
    ...
    def select_action(self, state):  # for discrete action space
        ...
        action = self.act(states)[0]
        if rd.rand() < self.explore_rate:  # epsilon-greedy
            a_prob = self.softmax(action).detach().cpu().numpy()  # choose action according to Q value
            a_int = rd.choice(self.action_dim, p=a_prob)
        else:
            a_int = action.argmax().detach().cpu().numpy()
        return a_int
...
class AgentPPO(AgentBase):
    ...
    def init(self, net_dim, state_dim, action_dim):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.compute_reward = self.compute_reward_gae if self.if_use_gae else self.compute_reward
    ...
    def select_action(self, state):  # states = (state, ...)
        states = torch.as_tensor((state,), dtype=torch.float32, device=self.device)
        a_noises, noises = self.act.get_action_noise(states)
        return a_noises[0].detach().cpu().numpy(), noises[0].detach().cpu().numpy()

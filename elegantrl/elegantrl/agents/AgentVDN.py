import copy
import torch
from elegantrl.agents.net import VDNMix


class AgentVDN:
    def __init__(self, mac, _scheme, logger, args):
        self.args = args
        self.mac = mac
        self.logger = logger
        ...
        if args.mixer is not None:
            assert args.mixer == "vdn"
            self.mixer = VDNMix()
            self.params += list(self.mixer.parameters())
            self.target_mixer = copy.deepcopy(self.mixer)

        self.optimiser = torch.optim.RMSprop(params=self.params, lr=args.lr, alpha=args.optim_alpha, eps=args.optim_eps)

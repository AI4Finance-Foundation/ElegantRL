...
class Evaluator:  # [ElegantRL.2021.10.13]
    def __init__(self, cwd, agent_id, marl,eval_env, eval_gap, eval_times1, eval_times2, target_return, if_overwrite):
    ...
    def evaluate_and_save_marl(self,  agent, steps, r_exp) -> (bool, bool):  # 2021-09-09
        ...
        if time.time() - self.eval_time < 2 ** 7 :
            if_reach_goal = False
            if_save = False
        else:
            self.eval_time = time.time()
            ...
            '''evaluate first time'''
            rewards_steps_list = [get_episode_return_and_step_marl(self.eval_env, agent) for _ in
                                  range(self.eval_times1)]
            r_avg, r_std, s_avg, s_std = self.get_r_avg_std_s_avg_std(rewards_steps_list)
            '''evaluate second time'''
            if r_avg > self.r_max:  # evaluate actor twice to save CPU Usage and keep precision
                rewards_steps_list += [get_episode_return_and_step_marl(self.eval_env, agent)
                                       for _ in range(self.eval_times2 - self.eval_times1)]
                r_avg, r_std, s_avg, s_std = self.get_r_avg_std_s_avg_std(rewards_steps_list)
    ...
    def draw_plot(self):
        ...
        if self.marl== False:
            ...
            save_learning_curve_marl(self.recorder, self.cwd, save_title)

def get_episode_return_and_step_marl(env, agent) -> (float, int):
    episode_step = 1
...
def save_learning_curve_marl(recorder=None, cwd='.', save_title='learning curve', fig_name='plot_learning_curve.jpg'):
    if recorder is None:

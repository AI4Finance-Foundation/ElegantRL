...
from beta0 import ChasingEnv, ChasingVecEnv
...
class Ensemble:
    ...
    def run(self, cwd, agent):
        ...
        '''load'''
        load_dir = self.get_load_dir_randomly(cwd)

        if load_dir:
            save_dir = f"{cwd}/pod_temp_{self.agent_id:04}"

            agent.traj_list[:] = [list() for _ in agent.traj_list]
            with DirLock(load_dir):
                recorder_copy_txt(load_dir, save_dir)
                agent.save_or_load_agent(load_dir, if_save=False)
                # print(f"|{'':20}Load: ID {self.agent_id}    r_avg {load_dir[-9:]}")

    def get_load_dir_randomly(self, cwd):
...
def recorder_copy_txt(src_dir, dst_dir):
    if not os.path.exists(src_dir):
        return
    for file_name in os.listdir(src_dir):
        if file_name[-4:] != '.pth':
            shutil.copy(f"{src_dir}/{file_name}",
                        f"{dst_dir}/{file_name}")
...
def recorder_load_last_line(recorder_path):
    last_list = get_last_list_of_a_file(recorder_path)
...
def server_evaluator(args):  # [ElegantRL.2021.12.12]
    ...
    for agent_id in cycle(range(ensemble_num)):
        ...
        '''print'''
        recorder_path = f'{src_path}/recorder.txt'
        if os.path.isfile(recorder_path):
            r_exp, step, total_step, logging_str = recorder_load_last_line(recorder_path)
        ...
        with DirLock(src_path):
            src_dir = f"{cwd}/pod_temp_{agent_id:04}"
            dst_dir = f"{cwd}/pod_eval_{r_avg:09.3f}"

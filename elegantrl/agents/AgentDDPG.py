...
class AgentDDPG(AgentBase):
    ...
    def __init__(self, net_dim, state_dim, action_dim, gpu_id=0, args=None):
        ...
        self.explore_noise_std = getattr(args, 'self.explore_noise', 0.1)  # explore noise of action
        self.act.explore_noise_std = self.explore_noise_std
        self.act_target.explore_noise_std = self.explore_noise_std
    ...
    def update_net(self, buffer) -> tuple:
        ...
        for i in range(update_times):
            ...
            if i % self.act_update_gap == 0:
                ...
                # action = self.act.get_action(state)  # policy gradient todo
...
class AgentDDPGHterm(AgentDDPG):
        ...
        if (buffer.next_p - buffer.prev_p) % buffer.max_capacity > 2 ** 11:
            with torch.no_grad():  # H term
                if buffer.prev_p <= buffer.next_p:
                    ...
                    buf_action = buffer.buf_action[buffer.prev_p:buffer.next_p]
                    buf_reward = buffer.buf_reward[buffer.prev_p:buffer.next_p]
                    buf_mask = buffer.buf_mask[buffer.prev_p:buffer.next_p]
                else:
                    buf_state = torch.vstack((buffer.buf_state[buffer.prev_p:], buffer.buf_state[:buffer.next_p],))
                    buf_action = torch.vstack((buffer.buf_action[buffer.prev_p:], buffer.buf_action[:buffer.next_p],))
                    buf_reward = torch.vstack((buffer.buf_reward[buffer.prev_p:], buffer.buf_reward[:buffer.next_p],))
                    buf_mask = torch.vstack((buffer.buf_mask[buffer.prev_p:], buffer.buf_mask[:buffer.next_p],))
    ...
    def get_obj_critic_raw(self, buffer, batch_size):
        ...
        with torch.no_grad():
            reward, mask, action, state, next_s = buffer.sample_batch(batch_size)
            next_action = self.act_target.get_action(next_s)
            next_q = self.cri_target(next_s, next_action)
            q_label = reward + mask * next_q
        q_value = self.cri(state, action)
        obj_critic = self.criterion(q_value, q_label)

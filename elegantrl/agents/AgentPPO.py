...
class AgentPPO(AgentBase):
    ...
    def explore_vec_env(self, env, target_step: int) -> list:
        ...
        ten_s = self.states if self.if_use_old_traj else env.reset()
        ...
        while i < target_step or not any(ten_dones):
            ...
            i += 1
            last_done[torch.where(ten_dones)[0]] = i  # behind `i+=1`
            ten_s = ten_s_next
    ...
    def update_net(self, buffer: ReplayBufferList):
        with torch.no_grad():
            ...
            buf_value = torch.cat([self.cri_target(buf_state[i:i + bs])
                                   for i in range(0, buf_state.shape[0], bs)], dim=0)

...
class AgentBase:  # [ElegantRL.2022.04.04]
    def __init__(self, net_dim: int, state_dim: int, action_dim: int, gpu_id=0, args=None):
        ...
        self.h_term_drop_rate = getattr(args, 'h_term_drop_rate', 2 ** -2)  # drop the data in H-term ReplayBuffer
        self.h_term_sample_rate = getattr(args, 'h_term_sample_rate', 2 ** -2)  # sample the data in H-term ReplayBuffer
    ...
    def get_r_sum_h_term(self, buf_reward, buf_mask) -> torch.Tensor:

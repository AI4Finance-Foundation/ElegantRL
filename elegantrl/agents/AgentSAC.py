...
class AgentModSAC(AgentSAC):  # Modified SAC using reliable_lambda and Two Time-scale Update Rule
    def __init__(self, net_dims: [int], state_dim: int, action_dim: int, gpu_id: int = 0, args: Config = Config()):
        super(AgentSAC).__init__(net_dims, state_dim, action_dim, gpu_id, args)
    ...
    def _update_objectives_raw(self, buffer: ReplayBuffer, batch_size: int, update_t: int) -> Tuple[float, float]:
        ...
        self.optimizer_backward(self.cri_optimizer, obj_critic)
        self.soft_update(self.cri_target, self.cri, self.soft_update_tau)
        self.critic_value = self.critic_tau * self.critic_value + (1 - self.critic_tau) * obj_critic.item()
        ...
        if self.update_a / update_t < 1 / (2 - reliable_lambda):  # auto Two-time update rule
            self.update_a += 1

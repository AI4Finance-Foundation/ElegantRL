...
class Actor(nn.Module):
    ...
    def get_logprob0(self, state, action): # todo get_logprob0
    ...
    def get_logprob(self, state, action):  # todo get_logprob2
        ...
        log_prob = action_std_log + self.log_sqrt_2pi + (action_tanh - action).pow(2).__mul__(0.5)
        log_prob += (-action_tanh.pow(2) + 1.000001).log()  # fix log_prob using the derivative of action.tanh()
        return log_prob

    def get_logprob3(self, state, action):  # todo get_logprob3
        action_avg = self.net(state)  # NOTICE! `action_avg` is a tensor without .tanh()
        action_std = self.explore_noise_std
        action_std_log = np.log(action_std)  # assert isinstance(action_std, float)
        ...
        log_prob = action_std_log + self.log_sqrt_2pi + (action_tanh - action).pow(2).__mul__(0.5)
        log_prob += (np.log(2.) - action_tanh - self.soft_plus(-2. * action_tanh)) * 2.  # better than below
        return log_prob
...
class ActorSAC(nn.Module):
    ...
    def get_action_logprob(self, state):
        ...
        log_prob += (-a_tan.pow(2) + 1.000001).log()  # fix log_prob using the derivative of action.tanh()
        return a_tan, log_prob.sum(1, keepdim=True)  # todo negative log_prob
...
class ActorFixSAC(nn.Module):
    ...
    def get_logprob(self, state, action):
        ...
        log_prob = action_std_log + self.log_sqrt_2pi + (a_noise - action).pow(2).__mul__(0.5)  # noise.pow(2) * 0.5
...
class ActorPPO(nn.Module):
    ...
    def get_logprob(self, state, action):
        ...
        delta = ((action_avg - action) / action_std).pow(2) * 0.5
        log_prob = -(self.action_std_log + self.log_sqrt_2pi + delta)  # new_logprob
        return log_prob

...
def compute_WSR_nn(user_weights, channel, precoder, noise_power, nr_of_users):
     ...
     for batch_index in range(nr_of_samples_per_batch):
        ...
        for user_index in range(nr_of_users):
            ...
            result = result + user_weights[batch_index][user_index]*(th.log(1 + user_sinr)/np.log(2.0))
...
def calc_wsr(self, channel_input, initial_tp, step_size):
    ...
    for loop in range(0,self.nr_of_iterations_nn):
        ...
        transmitter_precoder = initial_transmitter_precoder
        ...
        for i in range(step_size.shape[0]):
            ...
            for i in range(self.nr_of_users):
                ...
                gradient.append(step_size[i] * (-2.0*a1_exp*a2_exp*th.matmul(channel[:,i,:,:],receiver_precoder[:,i,:,:])+ 2*th.matmul(sum_gradient,initial_transmitter_precoder[:,i,:,:]))) 
...
if __name__    == "__main__":
    ...
    net = Net([1.0, 1.0, 1.0, 1.0])

    optimizer = th.optim.Adam(net.parameters(), lr=0.0005)
    print("start of session")
    start_of_time = time.time()
    

    try:
        for i in range(nr_of_batches_training * 0):
            batch_for_training = []
            ...
            # Training
            channel_input = th.as_tensor(batch_for_training)
    ...
    for i in range(nr_of_batches_test):        
        ...
        initial_tp = th.as_tensor(initial_transmitter_precoder_batch)
        net.load_state_dict(th.load('./Hopper-v2_DDPG_0/actor__000001267000_07897.260.pth', map_location=th.device("cuda:0")))
        step = net()
        step = step[0]

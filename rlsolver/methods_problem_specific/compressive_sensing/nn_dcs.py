...
def train_dcs(latent_dim=100, batch_size=64, m_dim=100, num_training_epoch=100000, lr=1e-4, initial_step_size=0.01, num_grad_iters=5, device=torch.device("cuda:0"), if_grad=False):
    ...
    for epoch in pbar:
        for i, (images, labels) in enumerate(training_data):
            ...
            if i % 50 == 0:
                ...
                desc = f"nbatch: {n_batch} | GEN_LOSS: {generated_loss:.2f} | RECON_LOSS: {RECON_LOSS:.2f} | step_size: {step_size.s.exp().item():.5f} | optim_cost: {(z_optimized - z_initial).square().sum(dim=-1).mean():.2f} | z_mean:{z_initial.norm(dim=-1).mean():.2f}|z_opt.mean: {z_optimized.norm(dim=-1).mean():.2f}"

...
plt.imsave('./fig/training_images.pdf', np.transpose(vutils.make_grid(real_batch[0].to(device)[:64], padding=2, normalize=True).cpu(),(1,2,0)))
...
class Discriminator(nn.Module):
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
            ...
            #nn.Conv2d(ndf * 4, ndf * 1, 4, 2, 1, bias=False),
...
for epoch in range(num_epochs):
    ...
    train_dst_shfl = train_dst[torch.randperm(60000)]
    nbatch= 0
    for i, data in enumerate(dataloader, 0):
        ...
        batch_size = b_size
        real_cpu = train_dst_shfl[nbatch * b_size:nbatch*b_size + b_size].reshape_as(real_cpu)
        ...
        if i % 50 == 0:
            ...
            torch.save(netG.state_dict(), f"./model/netG.pth")
            torch.save(netD.state_dict(), f"./model/netD.pth")
...
plt.ylabel("Loss")
plt.legend()
plt.savefig("./fig/Loss_iterations.pdf")
#plt.show()
...
for i in img_list:
    #print(i)
    plt.imsave(f'./fig/{i}.png',np.transpose(i.numpy(),(1,2,0)))
ims = [[plt.imshow(np.transpose(i,(1,2,0)), animated=True)] for i in img_list]
ani = animation.ArtistAnimation(fig, ims, interval=1000, repeat_delay=1000, blit=True)
#writervideo = animation.FFMpegWriter(fps=60)
ani.save('./fig/animation.mp4')
#HTML(ani.to_jshtml())
...
plt.imsave('./fig/real_image.png', np.transpose(vutils.make_grid(real_batch[0].to(device)[:64], padding=5, normalize=True).cpu().numpy(),(1,2,0)))

# Plot the fake images from the last epoch
plt.subplot(1,2,2)
plt.axis("off")
plt.title("Fake Images")
plt.imsave('./fig/fake_image.png', np.transpose(img_list[-1].numpy(),(1,2,0)))
plt.show()
